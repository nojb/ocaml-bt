...
*)
  ...
  let g =
    { Dh.p = Z.of_cstruct_be @@ Cs.of_hex dh_prime; gg = Z.of_int 2; q = None }

  let priv, pub = Dh.gen_key g

  type encryption_mode = Encrypted | Both | Plain
  ...
  let _select mode crypto_provide =
    match (mode, crypto_provide) with
    | Plain, 0x01l | Plain, 0x03l -> Some 0x01l
    | Encrypted, 0x2l | Encrypted, 0x3l | Both, 0x2l | Both, 0x3l -> Some 0x2l
    | Both, 0x1l -> Some 0x1l
    | _ -> None

  let provide = function Plain -> 0x01l | Encrypted -> 0x02l | Both -> 0x03l
  ...
  let arc4 =
    ...
    fun from secret skey ->
      ...
      let { ARC4.key; _ } = ARC4.encrypt ~key discard in
      key
  ...
  let handle_client t state =
    let rec loop buf st =
      ...
      | ClientWaitDh when len >= Dh.modulus_size g ->
          let _yb, buf = Cstruct.split buf (Dh.modulus_size g) in
          (* let shared_secret = Dh.shared g priv yb in *)
          let shared_secret = assert false in
          (* FIXME *)
          ...
          let cs =
            Cstruct.concat
              [
                ...
                Cs.xor
                  (SHA1.digestv [ req2; t.info_hash ])
                  (SHA1.digestv [ req3; shared_secret ]);
                message;
              ]
          in
          `Ok
            ( { t with buf; state = Client (ClientSyncVC (my_key, her_key, 0)) },
              Some cs )
      | ClientWaitDh -> `Ok (t, None)
      | ClientSyncVC (_, _, off) when off >= max_pad_len ->
          `Error "couldn't find VC"
      | ClientSyncVC (_, _, off) when off + Cstruct.length vc > len ->
          `Ok (t, None)
      | ClientSyncVC (my_key, her_key, off) ->
          let { ARC4.key = her_key; message = vc' } =
            ARC4.decrypt ~key:her_key (Cstruct.sub buf off (Cstruct.length vc))
          in
          if Cstruct.equal vc' vc then
            let buf = Cstruct.shift buf (Cstruct.length vc) in
            loop buf (ClientWaitCryptoSelect (my_key, her_key))
          else loop buf (ClientSyncVC (my_key, her_key, off + 1))
      | ClientWaitCryptoSelect (my_key, her_key) when len >= 6 ->
          let message, buf = Cstruct.split buf 6 in
          let { ARC4.key = her_key; message } =
            ARC4.decrypt ~key:her_key message
          in
      ...
      | ClientWaitCryptoSelect _ -> `Ok (t, None)
      | ClientWaitPadD (my_key, her_key, crypto_select, padD_len)
        when len >= padD_len ->
          let padD, buf = Cstruct.split buf padD_len in
          let { ARC4.key = her_key; _ } = ARC4.decrypt ~key:her_key padD in
          let mode =
          ...
          in
          `Success (mode, buf)
      | ClientWaitPadD _ -> `Ok (t, None)
    in

    loop t.buf state

  let handle_server _ _ = assert false
  ...
  let handle t buf =
    ...
    match t.state with
    | Client state -> handle_client t state
    | Server state -> handle_server t state
  ...
  let _incoming ~info_hash:_ _mode = `Error "not implemented"

  type result = (* | Ok of Util.socket * Cstruct.t *)
    | Failed
  ...
  let negotiate sock t =
    ...
    let rec loop = function
      ...
      | `Ok (t, None) -> (
          sock#read read_buf >>= function
          | 0 -> Lwt.fail End_of_file
          | n -> loop (handle t (Cstruct.sub read_buf 0 n)))
      | `Error err -> Lwt.fail (Failure err)
      | `Success (None, rest) -> Lwt.return (sock, rest)
      | `Success (Some (enc, dec), rest) ->
          Lwt.return (new Util.encrypt sock enc dec, rest)
    in
    loop t

  let outgoing ~info_hash sock mode = negotiate sock (outgoing ~info_hash mode)
end
...
let _dht_bit = 63 (* last bit of the extension bitfield *)
...
let handshake_len =
  Cstruct.length proto + 8 (* extensions *) + 20 (* info_hash *) + 20
(* peer_id *)
...
let parse_handshake cs =
  if Cstruct.length cs != handshake_len then invalid_arg "parse_handshake";
  ...
  | 19 ->
      ...
      if Cstruct.equal proto' proto then
        ...
        `Error
          (Printf.sprintf "unknown protocol: %S" (Cstruct.to_string proto'))
  | n -> `Error (Printf.sprintf "bad protocol length %d" n)

let outgoing_handshake ~id ~info_hash sock rest =
  Lwt_cstruct.complete sock#write (handshake_message ~id ~info_hash)
  >>= fun () ->
  assert (Cstruct.length rest <= handshake_len);
  let hs = Cstruct.create handshake_len in
  Cstruct.blit rest 0 hs 0 (Cstruct.length rest);
  Lwt_cstruct.complete sock#read (Cstruct.shift hs (Cstruct.length rest))
  >>= fun () ->
  match parse_handshake hs with
  | `Ok (ext, info_hash', peer_id) ->
      if SHA1.equal info_hash info_hash' then Lwt.return (sock, ext, peer_id)
      else Lwt.fail (Failure "bad info-hash")
  | `Error err -> Lwt.fail (Failure err)

type kind = Plain | Encrypted
...
let outgoing kind ~id ~info_hash addr =
  let ip, port = addr in
  let f ~info_hash sock =
    match kind with
    | Encrypted ->
        Encryption.outgoing ~info_hash:(SHA1.to_raw info_hash) sock
          Encryption.Both
    | Plain -> Lwt.return (sock, Cs.empty)
  in
  let connect fd =
    Logs_lwt.info (fun m ->
        m "Connecting [%s] to %s:%d..."
...
let outgoing ~id ~info_hash addr push =
  Lwt.try_bind
    (fun () -> outgoing Plain ~id ~info_hash addr)
    (fun (sock, ext, peer_id) -> Lwt.wrap1 push @@ Ok (sock, ext, peer_id))
    (function
      | e ->
          Logs_lwt.err (fun m -> m "%s" (Printexc.to_string e)) >>= fun () ->
